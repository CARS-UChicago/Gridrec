...
void setop(char c) {
	...
	case 'w':
		...
		if (fexist(optarg)) {
		    wf = readPMIS(optarg, &WfHd, &WfX, &WfY);
...
close(afd);
	...
	if (contin) {
	    ...
	    sprintf(fileName, "%s%03d%s", fnPrefix, ++fnNumb, fnSuffix);
	    fangle = fangle + fnDelT;
	} else {
	    switch(inchar) {
		case '#':
		    if (fscanf(in, "%s %d %d %s %g %g\n", 
		    fnPrefix, &fnNumb, &fnNumbMax, fnSuffix, 
		    &fangle, &fnDelT) != 6) {
	...
	view = readPMIS(fileName, &viewHd, &xmax, &ymax);
