...
void recon_init(grid_struct *GP,sg_struct *SGP)
...

{
	*imgsiz=M0;
	...
	if(verbose)printf(
	"do_recon(): M0=%d M= %d pdim=%d L=%f scale=%f\n",
	M0,M,pdim,L,scale);
...
Loop over the n_ang projection angles. For each angle, do the following:
...
***********************************************************************/
	...
	float convolvx,convolv,tblspcg=2*ltbl/L;
